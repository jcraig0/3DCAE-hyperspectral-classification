...
def DCAE_v2(data_name, weight_decay=0.0005):
    ...
    model.add(Conv3D(filters=24,
                     ...
                     kernel_size=((24, 2, 0)[dst_index], 3, 3),
                     strides=(1, 1, 1),
    ...
    model.add(MaxPool3D(pool_size=((18, 5, 0)[dst_index], 1, 1),
                        strides=(5, 1, 1), name="Pool1"))

    model.add(Conv3DTranspose(filters=24,
                              kernel_size=((9, 10, 0)[dst_index], 3, 3),
                              kernel_regularizer=regularizers.l2(
                                  l=weight_decay),
                              strides=((22, 1, 0)[dst_index], 1, 1), name="Deconv1", padding='valid'))
    model.add(BatchNormalization(name="BN3"))
    model.add(PReLU(name="PReLU3"))
    model.add(Conv3DTranspose(filters=1,
                              kernel_size=((27, 2, 0)[dst_index], 3, 3),
                              kernel_regularizer=regularizers.l2(
                                  l=weight_decay),
